...
To validate the moving immersed boundary method, we compare against the oscillating cylinder experiments of \citep{aktosun2024observed}, with both cross-stream and inline motions:
\begin{equation*}
    y = A_y\sin(\omega t), \quad x = A_x\sin(2\omega t+\theta)
\end{equation*}
and we pick a large amplitude cases where $A_y=1.6D$, $A_x=0.4D$, $\theta=\pi/6$ and $\omega=\frac{2\pi}{5.4} U/D$ where $U,D$ are in flow velocity (in the $x$ direction) and cylinder diameter, respectively and the Reynolds number is $Re_D=7620$. We use a domain of $3n \times 2n \times n$, where $n=2^7$ cells, giving 12.5M cells overall. The domain boundary conditions are periodic in $z$, reflection in $y$, and inlet/exit conditions in $x$. The cylinder spans the full $z$ domain, is place $n$ cells away from the side and inlet boundaries and has a diameter of $D=n/5$. Figure \ref{fig:cyl_val}(a) shows the cylinder path and resulting vorticity field showing a classic 2S wake.  Figure \ref{fig:cyl_val}(b) shows the measured force and power coefficients, along with the phase-averaged experimental measurements force coefficients. The agreement is excellent, and the mean power coefficient of $C_p=-4.54$ is within 5\% of the measure mean of -$4.52$ for this case. 
