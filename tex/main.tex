...
% \begin{figure}
%     \centering
%     \includegraphics[width=0.32\linewidth,trim={90 150 70 200},clip]{img/Whale_1.png}
%     \includegraphics[width=0.32\linewidth,trim={90 150 70 200},clip]{img/Whale_2.png}
%     \includegraphics[width=0.32\linewidth,trim={90 150 70 200},clip]{img/Whale_3.png}
%     \includegraphics[width=0.32\linewidth,trim={90 150 70 200},clip]{img/Whale_4.png}

%     \caption{Flow induced by a flapping whale tail geometry visualized by vorticity magnitude at equally spaced intervals over a cycle using cord resolution $L=96$ and sweep $s=10$.}
%     \label{fig:whale}
% \end{figure}
\begin{jllisting}
...
The tail is a planar membrane define using the ParametricBodies package \cite{WeymouthLauber2023}. The planform is defined by a set of points which are interpolated using a cubic spline. The sweep of the wing is adjustable with the input parameter $s$, allowing geometry parametric studies to be carried out with ease, as in \cite{zurman2021}. The 3D distance function and normal to this planar membrane are then evaluated using a parametric root-finding method to immerse the geometry in the simulation as with the examples above. Harmonic pitch and heave motion are used to flap the tail. Fig~\ref{fig:whale} shows equally spaced snapshots of the geometry and resulting flow throughout the cycle, matching the vortex structures found in previous works \cite{zurman2021}.
