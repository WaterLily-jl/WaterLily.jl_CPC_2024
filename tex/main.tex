...
\end{equation*}
and we pick a large amplitude cases where $A_y=1.6D$, $A_x=0.4D$, $\theta=\pi/6$ and $\omega=\frac{2\pi}{5.4} U/D$ where $U,D$ are in flow velocity (in the $x$ direction) and cylinder diameter, respectively and the Reynolds number is $Re_D=7620$. We use a domain of $4n \times 2n \times n$, where $n=2^7$ cells, giving 16.7M cells overall. The domain boundary conditions are periodic in $z$, reflection in $y$, and inlet/exit conditions in $x$. The cylinder spans the full $z$ domain, is place $n$ cells away from the side and inlet boundaries and has a diameter of $D=n/4$. Figure \ref{fig:cyl_val}(a) shows the cylinder path and resulting vorticity field showing an extremely energetic 3D wake.  Figure \ref{fig:cyl_val}(b) shows the measured force and power coefficients, along with the phase-averaged experimental measurements force coefficients. The agreement is excellent, and the mean power coefficient of $C_p=-4.49$ is within 1\% of the measure mean of -$4.52$ for this case.}
